...
def _dg08(d: _Data) -> dict:
    ...
    return {"consumer_total_revenue": round(by_segment["Consumer"], 2),
            ...
            "_note": "Corporate and Consumer segment revenue are nearly identical (within 0.4%) -- "
                     "there is no significant gap to explain"}
...
def _dg09(d: _Data) -> dict:
    ...
    return {"display_ctr": round(by_channel["Display"], 4), "email_ctr": round(by_channel["Email"], 4),
            ...
            "_note": "Display's CTR (2.70%) is essentially identical to every other channel (2.70%-2.79%) -- "
                     "there is no meaningful underperformance to explain"}
...
def _dg13(d: _Data) -> dict:
    ...
    return {"furniture_revenue": round(by_cat_rev["Furniture"], 2), "books_revenue": round(by_cat_rev["Books"], 2),
            ...
            "_note": "Books actually generates slightly MORE total revenue than Furniture ($3,112,756 vs "
                     "$3,071,246) as well as a higher margin -- there is no revenue-vs-margin tradeoff to explain here"}
...
def main():
    ...
    for cid in case_ids:
        ...
        # not a real mismatch); string/_note keys must match exactly.
        diffs = {}
