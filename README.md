...
- **MCP protocol** for sub-system communication — designed so MAEDA delegates rather than reimplements data-quality profiling and domain retrieval. **Currently exercised 0% of the time in this environment** — Data Cleaner and RAG-MCP-Server have never been reachable here, so every run falls back to a local pandas profiler and skips RAG enrichment. The fallback path is real and tested, but it's the *only* path that's actually run so far, not a rare degradation case.
